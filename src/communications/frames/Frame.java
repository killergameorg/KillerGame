...
/**
 * The abstract base class for communication frames. A frame represents a unit
 * of data that is exchanged between two communication endpoints. A frame
 * consists of a header and a payload. The header contains metadata such as the
 * unique identifier of the frame, the time-to-live (TTL) value, and the source
 * and target IP addresses. The payload contains the actual data being
 * exchanged. This class provides methods to set and get the header and payload
 * fields, as well as to decrement the TTL value and determine whether the TTL
 * has expired. Concrete subclasses of this class implement specific types of
 * frames, such as request frames, response frames, or error frames, each with
 * their own set of fields and behavior.
 * 
